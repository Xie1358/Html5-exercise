...
function openDB() {
    ...
    //使用open()建立資料庫
    var request = window.indexedDB.open("TestDB",1);     //請將 "_______" 換成正確的程式
    ...
    //在此事件中我們可以建立Object Store 來存放資料,此種on開頭是較舊寫法
    request.onupgradeneeded = function (e) {      //請將 "_______" 換成正確的程式
        ...
        if (db.objectStoreNames.contains("emp")) {
            ...
            var store = db.createObjectStore("emp",{"keyPath":"myKey"});   //請將 "_______" 換成正確的程式

        //     //使用createIndex的方法建立name,email的key及index
        //   ____________________________________________________________;    //請將 "_______" 換成正確的程式
        //     ____________________________________________________________;    //請將 "_______" 換成正確的程式
        }
    };
...
function writeItem() {
    ...
    //使用add的方法新增一筆資料
    var writeRequest = store.add({"myKey":key,"name":name,"email":email}); //請將 "_______" 換成正確的程式
...
function removeItem(key) {
    ...
    var store = trans.objectStore("emp");
    //使用delete的方法進行刪除
    var deleteRequest = store.delete(parseInt(key)); //請將 "_______" 換成正確的程式  
